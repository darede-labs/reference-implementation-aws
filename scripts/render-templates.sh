...
if [ -f "platform/keycloak/bootstrap/realm-configmap.yaml" ] && [ -f "platform/keycloak/realm-config.json" ]; then
    info "Injecting realm config into ConfigMap..."
    # Create a temporary file with the ConfigMap header
    grep -B 100 "REALM_CONFIG_PLACEHOLDER" platform/keycloak/bootstrap/realm-configmap.yaml | grep -v "REALM_CONFIG_PLACEHOLDER" > platform/keycloak/bootstrap/realm-configmap.yaml.tmp
    # Add the realm.json key with proper YAML literal block syntax
    echo "  realm.json: |" >> platform/keycloak/bootstrap/realm-configmap.yaml.tmp
    # Indent the JSON content by 4 spaces for YAML
    sed 's/^/    /' platform/keycloak/realm-config.json >> platform/keycloak/bootstrap/realm-configmap.yaml.tmp
    # Move the temp file to the final location
    mv platform/keycloak/bootstrap/realm-configmap.yaml.tmp platform/keycloak/bootstrap/realm-configmap.yaml
fi
